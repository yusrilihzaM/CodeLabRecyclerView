package com.yusril.codelabrc.data
...
object HeroesData {
    ...
    val listData: ArrayList<Hero>get() {
        ...
        for(position in heroNames.indices){
            val hero = Hero()// panggil model
            hero.name = heroNames[position]// panggil nama sesuai index
            hero.detail = heroDetails[position]// panggil detail sesuai index
            hero.photo = heroesImages[position]// panggil phoyo sesuai index
            list.add(hero)// masukan data kedalam list [nama, detail, photo]
        }
        return list
    }
}